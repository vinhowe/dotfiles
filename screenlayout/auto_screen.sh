...
then
    ...
    # xrandr --auto && xrandr --output HDMI1 --primary --mode 2560x1440 --pos 0x0 --output eDP1 --off
    # xrandr --auto && xrandr --output HDMI1 --primary --mode 2560x1440 --dpi 192 --rotate normal --pos 0x0
    ...
    /usr/bin/notify-send "HDMI active" "Switching to docked mode"
else
    echo [$(date -u)] "setting mode to mobile"

    xrandr --auto && xrandr --output HDMI1 --off
    xrandr --output eDP1 --primary --dpi 96 --mode 1366x768 --scale 1x1
    ...
    /usr/bin/notify-send "HDMI inactive" "Switching to mobile mode"
fi
